...
bun run db:push || echo "[BUN] Warning: Database push failed (DB may be unreachable). Continuing..."
...
curl -s -o /dev/null -w "%{http_code}" localhost:3000 | grep -qE "200|404" && echo "[BUN] Health check passed" || echo "[BUN] Health check: server responding"
